<template>
 <div class="wm_scrollwindow">
      ...
      <div v-if="!$root.shadowDB.Timeline[$route.params.id]">loading ...</div>
      <div v-else>
        <div>
          <div cols="12" sm="12" md="6">
            <input
              type="text"
        ...
        <section id="cd-timeline" class="cd-container">
          <draggable
            v-model="$root.shadowDB.Timeline[$route.params.id].content"
            handle=".cd-location"
            @change="SaveChange()"
          >
            <transition-group type="transition" :name="'flip-list'">
              <div
                ...
                v-for="(card, index) in $root.shadowDB.Timeline[
                  $route.params.id
                ].content"
                :key="index"
              >
                ...
                <div class="cd-timeline-content">
                  <div class="card" >
                    <div style="padding: 10px">
                      ...
                      >
                        <i class="mdi mdi-delete md-24"></i>
                      </button>
                    </div>
                    <div>
                      <CardEditor
...
export default {
  ...
  methods: {
    AddTimelineNode() {
      this.$root.shadowDB.Timeline[this.$route.params.id].content.push({
    ...
    removeMe(index) {
      if (confirm("you sure?")) {
        this.$root.shadowDB.Timeline[this.$route.params.id].content.splice(
          index,
          1
        );
        this.SaveChange();
